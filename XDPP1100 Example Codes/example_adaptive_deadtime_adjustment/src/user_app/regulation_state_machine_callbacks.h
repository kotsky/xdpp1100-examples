...
typedef struct {
	...
	int32_t iin[NUM_LOOPS];		//!< current telemetry value for input current of each loop
	uint32_t vin[NUM_LOOPS];	//!< current telemetry value for input voltage of each loop
	uint32_t pout[NUM_LOOPS];	//!< current telemetry value for output power of each loop
	uint32_t pin[NUM_LOOPS];	//!< current telemetry value for input power of each loop
	uint32_t fsw[NUM_LOOPS];	//!< current telemetry value for frequency switch of each loop
	uint32_t duty[NUM_LOOPS];	//!< current telemetry value for duty cycle of each loop
	uint32_t prebias_voltage[NUM_LOOPS];	//!< current telemetry value for pre-bias voltage at startup
	int32_t leakage_current[NUM_LOOPS];		//!< current telemetry value for leakage current at startup
	...
	/* Adaptive Deadtime Adjustment Example Modification Start */
	uint16_t deadtime_curr_region; // for adaptive deadtime adjustment. To store deadtime current region
	uint16_t deadtime_prev_region; // for adaptive deadtime adjustment. To store deadtime current region
	/* Adaptive Deadtime Adjustment Example Modification End */
